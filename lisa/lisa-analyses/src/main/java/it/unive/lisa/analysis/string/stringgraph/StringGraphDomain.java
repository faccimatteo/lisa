...
public class StringGraphDomain extends BaseNonRelationalValueDomain<StringGraphDomain> {
	 ...
	 * 
	 * @param stringGraph string graph needed to build
	 *                        {@code StringGraphDomain}.
	 */
	...
	@Override
	public StringGraphDomain evalTernaryExpression(TernaryOperator operator, StringGraphDomain left,
			StringGraphDomain middle, StringGraphDomain right, ProgramPoint pp) throws SemanticException {
		if (operator instanceof StringSubstring) {
			return new StringGraphDomain(
					left.stringGraph.substring(middle.stringGraph.getBound(), right.stringGraph.getBound()));
		}
		return new StringGraphDomain(StringGraph.buildMAX());
	}

	@Override
	public SemanticDomain.Satisfiability satisfiesBinaryExpression(BinaryOperator operator, StringGraphDomain left,
			StringGraphDomain right, ProgramPoint pp) throws SemanticException {
	...
	public StringGraphDomain lubAux(StringGraphDomain other) {
		StringGraph lubGraph = new StringGraph(StringGraph.NodeType.OR,
				new ArrayList<>(List.of(this.stringGraph, other.stringGraph)), null);
		lubGraph.compact();
		lubGraph.normalize();
	...
	/**
	 * Auxiliary widening algorithm applying the {@code cycleInductionRule} and
	 * the {@code replacementRule}.
	 * 
	 * @param other {@code StringGraphDomain} to compare during the widening
	 *                  application process.
	 * 
	...
	private StringGraphDomain widen(StringGraphDomain other) {
		...
		if (Objects.nonNull(gResCI))
			return this.widen(this.lubAux(new StringGraphDomain(gResCI)));
		else if (Objects.nonNull(gResCR))
			return this.widen(this.lubAux(new StringGraphDomain(gResCR)));
		else
			return other;
	}
