...
public class StringGraph {
     ...
     * @param c character to check presence
     * @return if condition in contains is {@code SATISFIED}, {@code NOT_SATISFIED} or {@code UNKNOWN}
     */
    ...
    public StringGraph cycleInductionRule(StringGraph other) {
        ...
        if (Objects.nonNull(nodesToCheck)) {
            other.replaceEdge(nodesToCheck.getLeft(), nodesToCheck.getRight());  // vertex from other is replaced with ancestor from other
            return other;
        }
        return null;
    }
     ...
     *
     * @param other string graph where we check for son-ancestor matching condition on {@code <i>replacementRuleAux</i>})
    ...
    public StringGraph replacementRule(StringGraph other) {
        ...
        if (Objects.nonNull(nodesToCheck)) {
            other.replaceVertex(nodesToCheck.getLeft(), nodesToCheck.getRight()); // ancestor is replaced
            return other;
        }
        return null;
    }
